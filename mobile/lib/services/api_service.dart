...
class ApiService {
  ...
  Future<Map<String, dynamic>> getAdvice(String disease) async {
    final uri = Uri.parse('${AppConfig.apiBaseUrl}/advice/$disease');
  ...
  Future<List<dynamic>> getDoctors(String disease) async {
    final uri = Uri.parse('${AppConfig.apiBaseUrl}/doctors/$disease');
    final response = await http.get(uri).timeout(AppConfig.apiTimeout);
